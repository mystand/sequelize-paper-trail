...
exports.init = function (sequelize, optionsArg) {
  ...
  _.extend(sequelize.Model, {
    hasPaperTrail: function hasPaperTrail() {
      ...
      this.prototype.getRevisionByNumber = async function (number) {
        const revision = await this.getRevisions({ where: { revision: number } })
        return revision.length > 0 ? (await revision[0].get({ plain: true }))['document'] : {}
      }
  ...
  var beforeHook = function beforeHook(instance, opt) {
    ...
    // Get diffs
    var isDiff = helpers.isDiff(previousVersion, currentVersion);
    var delta = null;
    if (options.enableRevisionChangeModel) {
      delta = helpers.calcDelta(previousVersion, currentVersion, options.exclude, options.enableStrictDiff)
    }      
    ...
    if (isDiff) {
      ...
      instance.context.isDiff = isDiff;

      // revision number
      var revisionId = (instance.get(options.revisionAttribute)  || 0) + 1;
  ...
  var afterHook = function afterHook(instance, opt) {
    ...
    if (instance.context && instance.context.isDiff) {
